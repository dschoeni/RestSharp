...
namespace RestSharp.Extensions
{
	...
	public static class ReflectionExtensions
	{
		...
		/// <returns></returns>
		public static object FindEnumValue(this Type type, string val)
		{
			var ret = Enum.GetValues( type )
			.Cast<Enum>()
			.FirstOrDefault(v => v.ToString().GetNameVariants().Contains(val, StringComparer.CurrentCulture));

			if (ret == null)
			{
				int enumValueAsInt;
				if (Int32.TryParse(val, out enumValueAsInt) && Enum.IsDefined(type, enumValueAsInt))
				{
