...
namespace RestSharp.Extensions
{
	public static class StringExtensions
	{
		...
		public static DateTimeOffset ParseJsonDate(this string input)
		{
			input = input.Replace("\n", "");
		...
		private static DateTime ParseFormattedDate(string input)
		{
			...
			DateTime date;
			if (DateTime.TryParseExact(input, formats, null, DateTimeStyles.None, out date))
			{
				return date;
			}

			if (DateTime.TryParse(input, null, DateTimeStyles.None, out date))
			{
				return date;
			}
		...
		private static DateTime ExtractDate(string input, string pattern)
		{
			...
			if (regex.IsMatch(input))
			{
				...
				{
					var mod = DateTime.ParseExact(match.Groups[3].Value, "HHmm", CultureInfo.CurrentCulture);
        ...
        public static IEnumerable<string> GetNameVariants(this string name)
		{
			...
			yield return name.ToCamelCase().AddUnderscorePrefix();
		}
	}
}
